...
/**
 * \file    core/triggers/interface_99_modCleaningservice_CleaningserviceTriggers.class.php
 ...
 *      - interface_99_modCleaningservice_MyTrigger.class.php
...
/**
 *  Class of triggers for Cleaningservice module
 */
class InterfaceCleaningserviceTriggers extends DolibarrTriggers
{
	/**
	 * Constructor
	 *
	...
	public function __construct($db)
	{
		...
		$this->family = "demo";
		$this->description = "Cleaningservice triggers.";
	...
	public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		// Replace the USER_LOGIN condition with:
		if ($action === 'USER_LOGIN') {
			// Remove the var_dump and check user groups
			require_once DOL_DOCUMENT_ROOT . '/user/class/usergroup.class.php';

			$usergroup = new UserGroup($this->db);
			$groups = $usergroup->listGroupsForUser($user->id);

			// Check if user is in housekeepers group (assumes group ID 3 is housekeepers)
			// You should replace '3' with your actual housekeeper group ID
			if (array_key_exists(1, $groups)) {
				header('Location: ' . DOL_URL_ROOT . '/custom/cleaningservice/schedule.php?mode=my?idmenu=298&mainmenu=cleaningservice&leftmenu=');
				exit;
			}
		}
		...
		switch ($action) {
			...
			case 'TASK_ASSIGNED':
				...
				$subject = $langs->transnoentities('TaskAssignmentEmailSubject', $object->ref);
				$message = $langs->transnoentities(
					'TaskAssignmentEmailBody',
					$object->ref,
					dol_print_date($object->date_start, 'dayhourtext'),
					dol_print_date($object->date_end, 'dayhourtext')
				);
