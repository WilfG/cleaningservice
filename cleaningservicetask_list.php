...
/**
 *   	\file       cleaningservicetask_list.php
 *		\ingroup    cleaningservice
 *		\brief      List page for cleaningservicetask
 */
...
require_once __DIR__.'/class/cleaningservicetask.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('cleaningservice', 'cleaningservicetask', 'write');
	$permissiontodelete = $user->hasRight('cleaningservice', 'cleaningservicetask', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("cleaningservice")) {
	accessforbidden('Module cleaningservice not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'CleaningServiceTask';
	$objectlabel = 'CleaningServiceTask';
	$uploaddir = $conf->cleaningservice->dir_output;
...
$title = $langs->trans("CleaningServiceTasks");
//$help_url = "EN:Module_CleaningServiceTask|FR:Module_CleaningServiceTask_FR|ES:Módulo_CleaningServiceTask";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/cleaningservice/cleaningservicetask_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-cleaningservice page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/cleaningservice/cleaningservicetask_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendCleaningServiceTaskRef";
$modelmail = "cleaningservicetask";
$objecttmp = new CleaningServiceTask($db);
